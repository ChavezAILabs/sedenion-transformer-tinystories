...
**Addendum 2026-07-24 (investigated across two sessions — Claude chat
side raised it, Claude Code verified against the real repo artifacts and
resolved the open question it left; not a strike-through correction
like §3(d) since the numbers above are accurate, this qualifies their
interpretation).** Two questions were raised about whether "X never
seeks the manifold" is well-founded: does X even *have* a zero-divisor
variety, and if so, is the ~100× vs ~2.2× fold-change comparison
confounded by unequal starting distances.

1. **X does have a zero-divisor variety — confirmed twice, independently, now on the real seeds.** Chat-side σ_min/SVD analysis and Claude Code's own gradient-descent check both found X's infimum indistinguishable from zero (Claude Code: true T16 known-pair sanity 5.7e-17; free GD search true T16 ~1e-3, X seeds ~1e-5 to 3e-5 — looser convergence than the chat side's L-BFGS+analytic-gradient run, ~1e-19, but the same qualitative answer and consistent with the *exact* zero the known ZD_PAIR already establishes for T16). **This dimension is generic, not sedenion-specific**: 30 ambient (S¹⁵×S¹⁵) − 16 equations = 14, true of *any* generic bilinear map R¹⁶×R¹⁶→R¹⁶ including a dense Gaussian one. Having a 14-dim ZD variety does not distinguish X from an arbitrary map and does not by itself distinguish it from the sedenions either — noted so this isn't later read as "X is secretly sedenion-like."
2. **Free-sphere accessibility reverses the naive framing — confirmed on the real seeds (`phase4_X_variety_sigma_min.py`), not just class-level: X's zero-divisor variety is ~493–497× *more* accessible than the true tensor's** under uniform sampling of unit x (median σ_min: true 0.477, X 0.040, all three seeds; **zero of 20,000 uniform samples fell below 1e-2 for the true tensor, versus ~13% for X**). Method validated against quaternions/octonions (infimum = 1.000000 exactly, as required for division algebras) and the known ZD_PAIR (session-close's own pair gives σ_min ≈ 5.7e-17). **If X's manifold is easier to reach in the abstract, "S > X because S's manifold is more accessible" is not an available explanation** — any surviving algebra-specific claim needs a different basis (e.g. coherence/associator structure), not zero-divisor availability.
3. **But this abstract accessibility does not describe what the model actually sees at initialization — checked directly (`phase4_init_r2_check.py`), and this is the load-bearing new result.** Free-sphere sampling draws raw unit vectors x,y; the model instead computes q,k via learned linear projections of embeddings, then rotates by the frozen R_8 ladder. Simulating fresh (untrained) instances of the real grid architecture (d_model=384, 6 heads, ctx=256 — `phase4_grid.GRID_DIMS`) and reading off the actual r² distribution the model would compute: **S's init minimum lands at 0.121–0.173 across 5 independent inits; X's lands at 0.044–0.092.** Both ranges bracket the real grid's own recorded step-0 values almost exactly (S: 0.1529, 0.1382; X: 0.0522, 0.0820 — a strong cross-check that this simulation is realistic, not a toy proxy). **Critically, X's simulated init r² distribution looks nothing like the free-sphere prediction** (median ≈0.97, not ≈0.04; 0 of a very large batch of pairs below 1e-2, not ~13%) **— it looks like S's.** The model's actual parameterization does not sample the algebra's sphere freely; it confines both variants to a "generic, far from any manifold" region at init, with only a modest ~2–3× gap in the minimum between S and X (nowhere near the ~13× median gap free-sphere sampling implied).

**Net reading:** the "unreconciled discrepancy" this raised (why does X's *training log* never cross r²<1e-2 if ~13% of free-sphere directions already sit there) is resolved by (3), not by explanation (2) or (3) of the three candidates originally listed — it's the parameterization, not the algebra's abstract accessibility, that governs what the model actually encounters. And because the real step-0 gap between S and X is small (~2–3×) while the *training-driven* gap is enormous (S descends 99–117×, X barely moves, 1.04–2.2×), **the "headroom" / baseline-asymmetry explanation for the original finding is not well supported once the real model's init distribution is used instead of free-sphere sampling — the original reading (S's training exploits its manifold, X's training does not) survives this scrutiny**, with two changes to how it should be described going forward: (i) don't describe X's manifold as hard to find in the abstract — it isn't; describe the finding as being about what training locates and exploits under the real parameterization, and (ii) don't attribute any surviving algebra-specific claim to zero-divisor *availability* — attribute it, if anything, to what happens once training engages with each algebra's manifold, which is a different and more defensible claim.
